...
async function main() {
  ...
  const targetUrl = 'https://saltanaa.vercel.app/ar/menu';

  // Output directory and file
  const outDir = path.join(process.cwd(), 'qr-output');
  const qrPath = path.join(outDir, 'saltana-qr-premium.png');
  ...
  const qrBuffer = await QRCode.toBuffer(targetUrl, {
    ...
    color: {
      dark: '#C9A56A', // Saltana gold
      light: '#080808', // deep obsidian
    },
  });
  ...
  const logoTile = await sharp({
    create: {
      ...
      height: logoTileSize,
      channels: 4,
      // very dark background to match brand
      background: { r: 5, g: 5, b: 5, alpha: 0.98 },
    },
  })
    .png()
    .composite([
        ...
        input: Buffer.from(
          `<svg width="${logoTileSize}" height="${logoTileSize}" viewBox="0 0 ${logoTileSize} ${logoTileSize}" xmlns="http://www.w3.org/2000/svg">
            <rect x="4" y="4" width="${logoTileSize - 8}" height="${logoTileSize - 8}" rx="${(logoTileSize - 8) / 4}" ry="${(logoTileSize - 8) / 4}"
              fill="url(#g)" stroke="#C9A56A" stroke-width="3" fill-opacity="0.08" />
            <defs>
              <radialGradient id="g" cx="50%" cy="30%" r="80%">
                <stop offset="0%" stop-color="#E8C98A" stop-opacity="0.25"/>
                <stop offset="50%" stop-color="#C9A56A" stop-opacity="0.08"/>
  ...
  const card = await sharp({
    ...
    .png()
    .composite([
      // outer gold border with slight gradient
      {
        input: Buffer.from(
          `<svg width="${cardWidth}" height="${cardHeight}" viewBox="0 0 ${cardWidth} ${cardHeight}" xmlns="http://www.w3.org/2000/svg">
            <defs>
              <linearGradient id="border" x1="0%" y1="0%" x2="100%" y2="100%">
                ...
                <stop offset="50%" stop-color="#C9A56A"/>
