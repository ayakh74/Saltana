...
export default function MenuItemForm({ categories, item, onSubmit }: Props) {
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-3">
            <F label="Arabic Name *" name="name_ar" required dir="rtl" defaultValue={item?.name_ar} placeholder="اسم الطبق" />
            <F label="Hebrew Name" name="name_he" dir="rtl" defaultValue={item?.name_he ?? ''} placeholder="שם המנה" />
            <F label="English Name" name="name_en" defaultValue={item?.name_en ?? ''} placeholder="Dish name" />
          </div>

          {/* Price + Tag + Order */}
          <div className="grid grid-cols-3 gap-3">
            <F label="Price (₪) *" name="price" type="number" required defaultValue={String(item?.price ?? '')} placeholder="0" />
            <div>
              <label className="block text-xs font-semibold tracking-widest uppercase text-cream/50 mb-2">Tag</label>
              <select name="tag" defaultValue={item?.tag ?? ''} className="w-full bg-obsidian-200 border border-gold/15 hover:border-gold/35 focus:border-gold/50 px-3 py-3 text-sm text-cream/80 outline-none transition-all">
