...
import { ImageOff, Star, Flame, Sparkles } from 'lucide-react';
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3 mb-2">
          <div className="text-right flex-1 min-w-0">
            <h3 className="text-cream/90 font-semibold text-base leading-snug">{item.nameAr}</h3>
            {item.nameHe && (
              <p className="text-cream/25 text-xs mt-0.5 font-heebo" dir="rtl">{item.nameHe}</p>
            )}
          </div>
          <div className="flex-shrink-0 text-left">
            <span className="text-gold-DEFAULT font-bold text-lg">
              {item.price}
            </span>
        ...
        {item.descAr && (
          <p className="text-cream/40 text-xs leading-relaxed text-right mt-2 line-clamp-2">
            {item.descAr}
          </p>
        )}
      </div>
  ...
  return (
    <div className="min-h-screen bg-obsidian pt-24">
      ...
      <div className="sticky top-[60px] z-40 bg-obsidian/95 backdrop-blur-xl border-y border-gold/8">
        <div
          ref={navRef}
          className="menu-scroll flex overflow-x-auto"
          style={{ scrollbarWidth: 'none' }}
        >
          <div className="flex items-center px-4 gap-1 py-1 min-w-max mx-auto">
            {menuData.map((cat) => {
              const label = t(`categories.${cat.key}` as 'categories.mains');
              ...
              return (
                <button
                  ...
                  className={cn(
                    'px-4 py-3 text-sm font-medium whitespace-nowrap transition-all duration-300 border-b-2 relative',
                    isActive
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12 space-y-20">
        {menuData.map((category) => {
          const label = t(`categories.${category.key}` as 'categories.mains');
          return (
            <section
              key={category.id}
              id={category.id}
            ...
            >
              ...
              <div className="flex items-center gap-6 mb-8 justify-end">
                ...
                <div className="flex items-center gap-3">
                  <h2 className="text-2xl sm:text-3xl font-black text-cream/90">{label}</h2>
