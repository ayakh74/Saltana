            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {reservations.map((r) => {
                  ...
                  const waLink = getWhatsAppLink(r.phone, `مرحباً ${r.name}، بخصوص حجزك في مطعم سلطنة بتاريخ ${r.date} الساعة ${String(r.time).slice(0,5)}`);
                  return (
                    <tr key={r.id} className="border-b border-gold/5 last:border-0 hover:bg-white/2 transition-colors">
                      ...
                      <td className="p-4 text-cream/40 max-w-[150px] truncate" title={r.notes ?? ''}>{r.notes || '—'}</td>
                      <td className="p-4">
